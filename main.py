...
class Pipeline:
    ...
    def insert_manuals_into_db(self, session: Session):
        ...
        for item in instructions:
            manual = Manual(id=item['id'], manual_title=item['manual_title'], txt_path=item.get('alias'))
            session.add(manual)
    ...
    def modify_pdf(self, pdf_path: str):
        ...
        try:
            ...
            pdf.Pages.RemoveAt(pdf.Pages.Count - 1)  # Remove the last page
...
def on_startup():
    ...
    pipeline = Pipeline()
    pipeline.fetch_data()  # Fetch data from API

    with Session(engine) as session:
        pipeline.insert_manuals_into_db(session)  # Insert manuals into DB
        pipeline.process_manuals(session)  # Process the manuals

@app.get("/manuals")
def get_manuals():
    """Fetches all manuals from the database."""
    with Session(engine) as session:
        manuals = session.exec(select(Manual)).all()
    return manuals
